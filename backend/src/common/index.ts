import { ECapabilityType, EPermissionType, EThermostatSubName, EThermostatTargetSetpointSubName, EThermostatWorkMode, EAdaptiveRecoveryStatus } from '@/enum';
import { ThermostatCapabilities } from '@/interface';

type State = Record<string, any>;

export const WEEKLY_SCHEDULE_PARAM_KEYS = ['mon', 'tues', 'wed', 'thur', 'fri', 'sat', 'sun'] as const;
export const WEEKLY_SCHEDULE_DAY_NAMES = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as const;

export const plainApiRegionDomainMap: Record<string, string> = {
  as: 'https://as-apia.coolkit.cc',
  cn: 'https://cn-apia.coolkit.cn',
  us: 'https://us-apia.coolkit.cc',
  eu: 'https://eu-apia.coolkit.cc',
};

export const dispatchLongLinkUrlMap: Record<string, string> = {
  as: 'https://as-dispa.coolkit.cc/dispatch/app',
  cn: 'https://cn-dispa.coolkit.cn/dispatch/app',
  us: 'https://us-dispa.coolkit.cc/dispatch/app',
  eu: 'https://eu-dispa.coolkit.cc/dispatch/app',
};

export const regionMap = [
  {
    'countryCode': '+86',
    'region': 'cn',
  },
  {
    'countryCode': '+93',
    'region': 'as',
  },
  {
    'countryCode': '+355',
    'region': 'eu',
  },
  {
    'countryCode': '+213',
    'region': 'eu',
  },
  {
    'countryCode': '+376',
    'region': 'eu',
  },
  {
    'countryCode': '+244',
    'region': 'eu',
  },
  {
    'countryCode': '+1264',
    'region': 'us',
  },
  {
    'countryCode': '+247',
    'region': 'eu',
  },
  {
    'countryCode': '+1268',
    'region': 'as',
  },
  {
    'countryCode': '+54',
    'region': 'us',
  },
  {
    'countryCode': '+374',
    'region': 'as',
  },
  {
    'countryCode': '+297',
    'region': 'eu',
  },
  {
    'countryCode': '+61',
    'region': 'us',
  },
  {
    'countryCode': '+43',
    'region': 'eu',
  },
  {
    'countryCode': '+994',
    'region': 'as',
  },
  {
    'countryCode': '+1242',
    'region': 'us',
  },
  {
    'countryCode': '+973',
    'region': 'as',
  },
  {
    'countryCode': '+880',
    'region': 'as',
  },
  {
    'countryCode': '+1246',
    'region': 'us',
  },
  {
    'countryCode': '+375',
    'region': 'eu',
  },
  {
    'countryCode': '+32',
    'region': 'eu',
  },
  {
    'countryCode': '+501',
    'region': 'us',
  },
  {
    'countryCode': '+229',
    'region': 'eu',
  },
  {
    'countryCode': '+1441',
    'region': 'as',
  },
  {
    'countryCode': '+975',
    'region': 'as',
  },
  {
    'countryCode': '+591',
    'region': 'us',
  },
  {
    'countryCode': '+387',
    'region': 'eu',
  },
  {
    'countryCode': '+267',
    'region': 'eu',
  },
  {
    'countryCode': '+55',
    'region': 'us',
  },
  {
    'countryCode': '+673',
    'region': 'as',
  },
  {
    'countryCode': '+359',
    'region': 'eu',
  },
  {
    'countryCode': '+226',
    'region': 'eu',
  },
  {
    'countryCode': '+257',
    'region': 'eu',
  },
  {
    'countryCode': '+855',
    'region': 'as',
  },
  {
    'countryCode': '+237',
    'region': 'eu',
  },
  {
    'countryCode': '+1',
    'region': 'us',
  },
  {
    'countryCode': '+238',
    'region': 'eu',
  },
  {
    'countryCode': '+1345',
    'region': 'as',
  },
  {
    'countryCode': '+236',
    'region': 'eu',
  },
  {
    'countryCode': '+235',
    'region': 'eu',
  },
  {
    'countryCode': '+56',
    'region': 'us',
  },
  {
    'countryCode': '+57',
    'region': 'us',
  },
  {
    'countryCode': '+269',
    'region': 'eu',
  },
  {
    'countryCode': '+242',
    'region': 'eu',
  },
  {
    'countryCode': '+243',
    'region': 'eu',
  },
  {
    'countryCode': '+682',
    'region': 'us',
  },
  {
    'countryCode': '+506',
    'region': 'us',
  },
  {
    'countryCode': '+225',
    'region': 'eu',
  },
  {
    'countryCode': '+385',
    'region': 'eu',
  },
  {
    'countryCode': '+53',
    'region': 'us',
  },
  {
    'countryCode': '+357',
    'region': 'eu',
  },
  {
    'countryCode': '+420',
    'region': 'eu',
  },
  {
    'countryCode': '+45',
    'region': 'eu',
  },
  {
    'countryCode': '+253',
    'region': 'eu',
  },
  {
    'countryCode': '+1767',
    'region': 'as',
  },
  {
    'countryCode': '+1809',
    'region': 'us',
  },
  {
    'countryCode': '+593',
    'region': 'us',
  },
  {
    'countryCode': '+20',
    'region': 'eu',
  },
  {
    'countryCode': '+503',
    'region': 'us',
  },
  {
    'countryCode': '+372',
    'region': 'eu',
  },
  {
    'countryCode': '+251',
    'region': 'eu',
  },
  {
    'countryCode': '+298',
    'region': 'eu',
  },
  {
    'countryCode': '+679',
    'region': 'us',
  },
  {
    'countryCode': '+358',
    'region': 'eu',
  },
  {
    'countryCode': '+33',
    'region': 'eu',
  },
  {
    'countryCode': '+594',
    'region': 'us',
  },
  {
    'countryCode': '+689',
    'region': 'as',
  },
  {
    'countryCode': '+241',
    'region': 'eu',
  },
  {
    'countryCode': '+220',
    'region': 'eu',
  },
  {
    'countryCode': '+995',
    'region': 'as',
  },
  {
    'countryCode': '+49',
    'region': 'eu',
  },
  {
    'countryCode': '+233',
    'region': 'eu',
  },
  {
    'countryCode': '+350',
    'region': 'eu',
  },
  {
    'countryCode': '+30',
    'region': 'eu',
  },
  {
    'countryCode': '+299',
    'region': 'us',
  },
  {
    'countryCode': '+1473',
    'region': 'as',
  },
  {
    'countryCode': '+590',
    'region': 'us',
  },
  {
    'countryCode': '+1671',
    'region': 'us',
  },
  {
    'countryCode': '+502',
    'region': 'us',
  },
  {
    'countryCode': '+240',
    'region': 'eu',
  },
  {
    'countryCode': '+44',
    'region': 'eu',
  },
  {
    'countryCode': '+224',
    'region': 'eu',
  },
  {
    'countryCode': '+592',
    'region': 'us',
  },
  {
    'countryCode': '+509',
    'region': 'us',
  },
  {
    'countryCode': '+504',
    'region': 'us',
  },
  {
    'countryCode': '+852',
    'region': 'as',
  },
  {
    'countryCode': '+95',
    'region': 'as',
  },
  {
    'countryCode': '+36',
    'region': 'eu',
  },
  {
    'countryCode': '+354',
    'region': 'eu',
  },
  {
    'countryCode': '+91',
    'region': 'as',
  },
  {
    'countryCode': '+62',
    'region': 'as',
  },
  {
    'countryCode': '+98',
    'region': 'ir',
  },
  {
    'countryCode': '+964',
    'region': 'as',
  },
  {
    'countryCode': '+353',
    'region': 'eu',
  },
  {
    'countryCode': '+44',
    'region': 'eu',
  },
  {
    'countryCode': '+972',
    'region': 'as',
  },
  {
    'countryCode': '+39',
    'region': 'eu',
  },
  {
    'countryCode': '+1876',
    'region': 'us',
  },
  {
    'countryCode': '+81',
    'region': 'as',
  },
  {
    'countryCode': '+44',
    'region': 'eu',
  },
  {
    'countryCode': '+962',
    'region': 'as',
  },
  {
    'countryCode': '+7',
    'region': 'eu',
  },
  {
    'countryCode': '+254',
    'region': 'eu',
  },
  {
    'countryCode': '+383',
    'region': 'eu',
  },
  {
    'countryCode': '+965',
    'region': 'as',
  },
  {
    'countryCode': '+996',
    'region': 'as',
  },
  {
    'countryCode': '+856',
    'region': 'as',
  },
  {
    'countryCode': '+371',
    'region': 'eu',
  },
  {
    'countryCode': '+961',
    'region': 'as',
  },
  {
    'countryCode': '+266',
    'region': 'eu',
  },
  {
    'countryCode': '+231',
    'region': 'eu',
  },
  {
    'countryCode': '+218',
    'region': 'eu',
  },
  {
    'countryCode': '+423',
    'region': 'eu',
  },
  {
    'countryCode': '+370',
    'region': 'eu',
  },
  {
    'countryCode': '+352',
    'region': 'eu',
  },
  {
    'countryCode': '+853',
    'region': 'as',
  },
  {
    'countryCode': '+389',
    'region': 'eu',
  },
  {
    'countryCode': '+261',
    'region': 'eu',
  },
  {
    'countryCode': '+265',
    'region': 'eu',
  },
  {
    'countryCode': '+60',
    'region': 'as',
  },
  {
    'countryCode': '+960',
    'region': 'as',
  },
  {
    'countryCode': '+223',
    'region': 'eu',
  },
  {
    'countryCode': '+356',
    'region': 'eu',
  },
  {
    'countryCode': '+596',
    'region': 'us',
  },
  {
    'countryCode': '+222',
    'region': 'eu',
  },
  {
    'countryCode': '+230',
    'region': 'eu',
  },
  {
    'countryCode': '+262',
    'region': 'eu',
  },
  {
    'countryCode': '+52',
    'region': 'us',
  },
  {
    'countryCode': '+373',
    'region': 'eu',
  },
  {
    'countryCode': '+377',
    'region': 'eu',
  },
  {
    'countryCode': '+976',
    'region': 'as',
  },
  {
    'countryCode': '+382',
    'region': 'as',
  },
  {
    'countryCode': '+1664',
    'region': 'as',
  },
  {
    'countryCode': '+212',
    'region': 'eu',
  },
  {
    'countryCode': '+258',
    'region': 'eu',
  },
  {
    'countryCode': '+264',
    'region': 'eu',
  },
  {
    'countryCode': '+977',
    'region': 'as',
  },
  {
    'countryCode': '+31',
    'region': 'eu',
  },
  {
    'countryCode': '+599',
    'region': 'as',
  },
  {
    'countryCode': '+687',
    'region': 'as',
  },
  {
    'countryCode': '+64',
    'region': 'us',
  },
  {
    'countryCode': '+505',
    'region': 'us',
  },
  {
    'countryCode': '+227',
    'region': 'eu',
  },
  {
    'countryCode': '+234',
    'region': 'eu',
  },
  {
    'countryCode': '+47',
    'region': 'eu',
  },
  {
    'countryCode': '+968',
    'region': 'as',
  },
  {
    'countryCode': '+92',
    'region': 'as',
  },
  {
    'countryCode': '+970',
    'region': 'as',
  },
  {
    'countryCode': '+507',
    'region': 'us',
  },
  {
    'countryCode': '+675',
    'region': 'as',
  },
  {
    'countryCode': '+595',
    'region': 'us',
  },
  {
    'countryCode': '+51',
    'region': 'us',
  },
  {
    'countryCode': '+63',
    'region': 'as',
  },
  {
    'countryCode': '+48',
    'region': 'eu',
  },
  {
    'countryCode': '+351',
    'region': 'eu',
  },
  {
    'countryCode': '+1',
    'region': 'us',
  },
  {
    'countryCode': '+974',
    'region': 'as',
  },
  {
    'countryCode': '+262',
    'region': 'eu',
  },
  {
    'countryCode': '+40',
    'region': 'eu',
  },
  {
    'countryCode': '+7',
    'region': 'eu',
  },
  {
    'countryCode': '+250',
    'region': 'eu',
  },
  {
    'countryCode': '+684',
    'region': 'us',
  },
  {
    'countryCode': '+685',
    'region': 'us',
  },
  {
    'countryCode': '+378',
    'region': 'eu',
  },
  {
    'countryCode': '+239',
    'region': 'eu',
  },
  {
    'countryCode': '+966',
    'region': 'as',
  },
  {
    'countryCode': '+221',
    'region': 'eu',
  },
  {
    'countryCode': '+381',
    'region': 'eu',
  },
  {
    'countryCode': '+248',
    'region': 'eu',
  },
  {
    'countryCode': '+232',
    'region': 'eu',
  },
  {
    'countryCode': '+65',
    'region': 'as',
  },
  {
    'countryCode': '+421',
    'region': 'eu',
  },
  {
    'countryCode': '+386',
    'region': 'eu',
  },
  {
    'countryCode': '+27',
    'region': 'eu',
  },
  {
    'countryCode': '+82',
    'region': 'as',
  },
  {
    'countryCode': '+34',
    'region': 'eu',
  },
  {
    'countryCode': '+94',
    'region': 'as',
  },
  {
    'countryCode': '+1869',
    'region': 'as',
  },
  {
    'countryCode': '+1758',
    'region': 'us',
  },
  {
    'countryCode': '+1784',
    'region': 'as',
  },
  {
    'countryCode': '+249',
    'region': 'eu',
  },
  {
    'countryCode': '+597',
    'region': 'us',
  },
  {
    'countryCode': '+268',
    'region': 'eu',
  },
  {
    'countryCode': '+46',
    'region': 'eu',
  },
  {
    'countryCode': '+41',
    'region': 'eu',
  },
  {
    'countryCode': '+963',
    'region': 'as',
  },
  {
    'countryCode': '+886',
    'region': 'as',
  },
  {
    'countryCode': '+992',
    'region': 'as',
  },
  {
    'countryCode': '+255',
    'region': 'eu',
  },
  {
    'countryCode': '+66',
    'region': 'as',
  },
  {
    'countryCode': '+670',
    'region': 'as',
  },
  {
    'countryCode': '+228',
    'region': 'eu',
  },
  {
    'countryCode': '+676',
    'region': 'us',
  },
  {
    'countryCode': '+1868',
    'region': 'us',
  },
  {
    'countryCode': '+216',
    'region': 'eu',
  },
  {
    'countryCode': '+90',
    'region': 'as',
  },
  {
    'countryCode': '+993',
    'region': 'as',
  },
  {
    'countryCode': '+1649',
    'region': 'as',
  },
  {
    'countryCode': '+256',
    'region': 'eu',
  },
  {
    'countryCode': '+380',
    'region': 'eu',
  },
  {
    'countryCode': '+971',
    'region': 'as',
  },
  {
    'countryCode': '+44',
    'region': 'eu',
  },
  {
    'countryCode': '+1',
    'region': 'us',
  },
  {
    'countryCode': '+598',
    'region': 'us',
  },
  {
    'countryCode': '+998',
    'region': 'as',
  },
  {
    'countryCode': '+678',
    'region': 'us',
  },
  {
    'countryCode': '+58',
    'region': 'us',
  },
  {
    'countryCode': '+84',
    'region': 'as',
  },
  {
    'countryCode': '+1340',
    'region': 'as',
  },
  {
    'countryCode': '+967',
    'region': 'as',
  },
  {
    'countryCode': '+260',
    'region': 'eu',
  },
  {
    'countryCode': '+263',
    'region': 'eu',
  },
];

export const initThermostatCapabilities: Array<ThermostatCapabilities> = [
  {
    capability: ECapabilityType.THERMOSTAT,
    permission: EPermissionType.READ_WRITE,
    name: EThermostatSubName.THERMOSTAT_MODE,
    configuration: {
      supportedModes: ['MANUAL', 'AUTO', 'ECO'],
    },
  },
  {
    capability: ECapabilityType.THERMOSTAT,
    permission: EPermissionType.READ,
    name: EThermostatSubName.ADAPTIVE_RECOVERY_STATUS,
  },
  {
    capability: ECapabilityType.THERMOSTAT_TARGET_SETPOINT,
    permission: EPermissionType.READ_WRITE,
    name: EThermostatTargetSetpointSubName.TEMPERATURE,
    configuration: {
      temperature: {
        min: 4,
        max: 35,
        increment: 0.5,
        scale: 'c',
      },
    },
  },
  {
    capability: ECapabilityType.THERMOSTAT_TARGET_SETPOINT,
    permission: EPermissionType.READ_WRITE,
    name: EThermostatTargetSetpointSubName.MANUAL_MODE,
    configuration: {
      temperature: {
        min: 4,
        max: 35,
        increment: 0.5,
        scale: 'c',
      },
      mappingMode: 'MANUAL',
    },
  },
  {
    capability: ECapabilityType.THERMOSTAT_TARGET_SETPOINT,
    permission: EPermissionType.READ,
    name: EThermostatTargetSetpointSubName.AUTO_MODE,
    configuration: {
      temperature: {
        min: 4,
        max: 35,
        increment: 0.5,
        scale: 'c',
      },
      mappingMode: 'AUTO',
      weeklySchedule: {
        maxEntryPerDay: 6,
        Monday: [],
        Tuesday: [],
        Wednesday: [],
        Thursday: [],
        Friday: [],
        Saturday: [],
        Sunday: [],
      },
    },
  },
  {
    capability: ECapabilityType.THERMOSTAT_TARGET_SETPOINT,
    permission: EPermissionType.READ_WRITE,
    name: EThermostatTargetSetpointSubName.ECO_MODE,
    configuration: {
      temperature: {
        min: 4,
        max: 35,
        increment: 0.5,
        scale: 'c',
      },
      mappingMode: 'ECO',
    },
  },
];

// ========== 状态 ↔ 参数 双向映射 ==========

export interface StateParamMapping {
  paramKey: string;
  get: (s: State) => any;
  set: (s: State, v: any) => void;
  // true: params→state  false: state→params
  transform: (value: any, isParams2State: boolean) => any;
}

const ST = EThermostatSubName;
const STSS = EThermostatTargetSetpointSubName;

export const thermostatStateParamMappings: StateParamMapping[] = [
  {
    paramKey: 'workMode',
    get: (s) => s.thermostat?.[ST.THERMOSTAT_MODE]?.thermostatMode,
    set: (s, v) => { s.thermostat[ST.THERMOSTAT_MODE].thermostatMode = v; },
    transform: (v, isParams2State) =>
      isParams2State ? EThermostatWorkMode[Number(v)] : EThermostatWorkMode[v as string].toString(),
  },
  {
    paramKey: 'workState',
    get: (s) => s.thermostat?.[ST.ADAPTIVE_RECOVERY_STATUS]?.adaptiveRecoveryStatus,
    set: (s, v) => { s.thermostat[ST.ADAPTIVE_RECOVERY_STATUS].adaptiveRecoveryStatus = v; },
    transform: (v, isParams2State) =>
      isParams2State ? EAdaptiveRecoveryStatus[Number(v)] : EAdaptiveRecoveryStatus[v as string].toString(),
  },
  {
    paramKey: 'manTargetTemp',
    get: (s) => s['thermostat-target-setpoint']?.[STSS.MANUAL_MODE]?.targetSetpoint,
    set: (s, v) => { if (!s['thermostat-target-setpoint'][STSS.MANUAL_MODE]) s['thermostat-target-setpoint'][STSS.MANUAL_MODE] = {}; s['thermostat-target-setpoint'][STSS.MANUAL_MODE].targetSetpoint = v; },
    transform: (v, isParams2State) => isParams2State ? v / 10 : v * 10,
  },
  {
    paramKey: 'autoTargetTemp',
    get: (s) => s['thermostat-target-setpoint']?.[STSS.AUTO_MODE]?.targetSetpoint,
    set: (s, v) => { if (!s['thermostat-target-setpoint'][STSS.AUTO_MODE])s['thermostat-target-setpoint'][STSS.AUTO_MODE] = {}; s['thermostat-target-setpoint'][STSS.AUTO_MODE].targetSetpoint = v; },
    transform: (v, isParams2State) => isParams2State ? v / 10 : v * 10,
  },
  {
    paramKey: 'ecoTargetTemp',
    get: (s) => s['thermostat-target-setpoint']?.[STSS.ECO_MODE]?.targetSetpoint,
    set: (s, v) => { if (!s['thermostat-target-setpoint'][STSS.ECO_MODE])s['thermostat-target-setpoint'][STSS.ECO_MODE] = {}; s['thermostat-target-setpoint'][STSS.ECO_MODE].targetSetpoint = v; },
    transform: (v, isParams2State) => isParams2State ? v / 10 : v * 10,
  },
];
